    ...
    Scaffold(
        topBar = {
            Surface(color = MaterialTheme.colorScheme.primary) {
                Row(
                    Modifier.statusBarsPadding().fillMaxWidth().padding(horizontal = 12.dp, vertical = 6.dp),
                    ...
                    Text(
                        if (vm.hideRu) " перевод скрыт" else " перевод виден",
                        color = MaterialTheme.colorScheme.onPrimary.copy(alpha = 0.85f),
                        fontSize = 12.sp
                    )
                    IconButton(onClick = { vm.toggleHideRu() }) {
                        Icon(
                            if (vm.hideRu) Icons.Filled.VisibilityOff else Icons.Filled.Visibility,
                            contentDescription = "Скрыть/показать русский перевод",
